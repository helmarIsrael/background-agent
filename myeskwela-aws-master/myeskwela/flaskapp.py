...
#production
app.config['UPLOAD_FOLDER'] ="/var/www/myeskwela/uploads" #"/home/ubuntu/myeskwela/myeskwela/uploads"
...
def filesdownload():
    ...
    try:
        ...
        return send_file(os.path.join(app.config['UPLOAD_FOLDER'],"filenotfound.jpg"))
...
def uploadimage():
    ...
    if len(lrn) == 0:
        ...
        if formatres(reslrnid)["status"] == "Error":
            return jsonify({"status": "error", "message": formatres(reslrnid)["message"]})
...
def inputgrade(par_studentid, par_entryid, par_score, par_mult, par_username, par_gradeinfo, par_token, par_group, par_semid, par_sectioncode):
    ...
    loc_entryinfo = spcall("getentryinfo", (par_entryid,))[0][0]

    if loc_entryinfo is None:
        return jsonify({
            "status":"error",
            "message" : "Score entry is not created! Click help button for guidance!"
        })
...
def batchscoreentry(par_entries, par_entryid, par_gradeinfo):
    ...
    try:
        for entry in par_entries.split("@"):
            stuid = entry.split("*")[0]
            score = entry.split("*")[1]
            item = inputgrade(stuid, par_entryid, score, 1.00, auth.username(), par_gradeinfo, token, group, semid, sectioncode)

            result.append({"lrn": stuid, "result": item["item"][0]})

        return jsonify({"results": result, "status": "ok"})
    except:
        return jsonify({"message": "Some inputs are not accepted!", "status": "error"})
